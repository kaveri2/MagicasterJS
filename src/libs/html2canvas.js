...
// but would require further work to calculate the correct positions for elements with offsetParents
    ...
    _html2canvas.Util.isTransparent = function(backgroundColor) {
        return (backgroundColor === undefined || backgroundColor === "transparent" || backgroundColor === "rgba(0, 0, 0, 0)");
    };
    ...
    _html2canvas.Parse = function (images, options, cb) {
        ...
        function addPseudoElements(el) {
            ...
            function getPseudoElementClasses(){
                ...
                for (i = classes.length -1 ; i >= 0; i--) {
                    ...
                    // JP Modification: Check if CSS class ends to unsupported
                    // character. If so, remove the whole class from processing.
                    // E.g. Zurb Foundation CSS styles would crash the logic otherwise.
                    classes[i] = classes[i].trim();
                    var lastChar = classes[i].charAt(classes[i].length - 1);
        ...
        function renderImage(ctx, element, image, bounds, borders, tMatrix, tOrigin) {
           ...
           // ctx.save();
            ...
            if (tMatrix[0] !== undefined && tMatrix[5] !== undefined) {
                console.log(tMatrix);
                ctx.setTransform(tMatrix[0], tMatrix[1], tMatrix[2], tMatrix[3], tMatrix[4], tMatrix[5]);
                ctx.setTransform(tMatrix[0], tMatrix[1], tMatrix[2], tMatrix[3], 0, 0);
                /*if (tOrigin && tOrigin[0] !== undefined && tOrigin[1] !== undefined) {
                    ctx.translate(tOrigin[0], tOrigin[1]);
                } */

                console.log( bounds.left + paddingLeft + borders[3].width);
                console.log( bounds.top + paddingTop + borders[0].width );
        ...
        function renderElement(element, parentStack, ignoreBackground) {
            ...
            // Get the scale factor from transform matrix
            //var _scalex = 1;
            //var _scaley = 1;
            var tMatrix = null;
            var tOrigin = null;

            var getScaling = function(stack) {
                if (stack.transform && stack.transform.matrix) {
                    if (!tMatrix) {
                        tMatrix = stack.transform.matrix;
                        console.log("origin");
                        console.log(stack.transform.origin);
                        tOrigin = stack.transform.origin;
                    }
                    else {
                        tMatrix.multiply(stack.transform.matrix);
                    }

                    //var scalx = stack.transform.matrix[0];
                    //var scaly = stack.transform.matrix[3];
                    //console.log("Found scalex: " + scalx);
                    //console.log("Found scaley: " + scaly);
                    //_scalex = _scalex * scalx;
                    //_scaley = _scaley * scaly;
                }
                if (stack.parentStack) {

                    // console.log("Parent found");
                    getScaling(stack.parentStack);
                    return true;
                }
                else{
                    return false;
            ...
            case "IMG":
                ...
                if ((image = loadImage(element.getAttribute('src')))) {
                    //console.log(tMatrix);
                    renderImage(ctx, element, image, bounds, borders, tMatrix, tOrigin);
                } else {
            ...
            case "CANVAS":
                getScaling(stack);
                renderImage(ctx, element, element, bounds, borders, tMatrix);
                break;
            case "VIDEO":
                ...
                testCtx.drawImage(element, 0, 0, w, h);

                getScaling(stack);
                renderImage(ctx, canvas, canvas, bounds, borders, tMatrix);
